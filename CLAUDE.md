...
- **Recording scope**: MS Teams by default (`ms-teams.exe`, `teams.exe`, `msteams.exe`). **Google Meet is greenlit as of 2026-07-29** and lives in `teams/meet.py`, armed per-machine with `NUCLEUS_ENABLE_MEET=1`. Never widen to Zoom / anything else without a fresh explicit greenlight.
- **Meet capture**: detection is browser mic-hold (Windows `ConsentStore`, `LastUsedTimeStop == 0`) **plus** a Meet window title, latched 300s. Browser audio alone is never the trigger — YouTube would fire it. Teams is evaluated first and always wins. One soundcard means one recording: overlapping Teams+Meet calls produce one mixed file, flagged `overlap` in the sidecar and warned about in the transcript header. Meet has no participant list, so `NUCLEUS_INCLUDE_MEMBERS` (the 7-person boundary) cannot apply to Meet calls; `NUCLEUS_MEET_TITLE_ALLOW` is the only narrowing available. Verify live with `py -3 -m teams.meet`.
...
1. **Call transcription engine = Google STT ONLY** (`tools/google_stt.py`, invoked from `collect_central._transcribe_call`). This is the sole, settled engine on central — decided 2026-06-08. Config lives in the central `.env` (`GOOGLE_STT_*`: language, alt-languages, model). Do NOT propose, install, or ask about any other engine — that decision is closed. Recording/audio quality is the real lever on transcript quality (see rule #6), not the engine.
