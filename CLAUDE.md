...
5. **Sender = `NAPCO Nucleus <napco-nucleus@ael-bd.com>`** (`SMTP_FROM`). This alias needs a verified Gmail "Send mail as" / Workspace alias on `khasan@`; until provisioned, Gmail rewrites the From to `khasan@ael-bd.com` (acceptable interim).
