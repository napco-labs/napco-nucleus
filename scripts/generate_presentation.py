...
def slide_title(prs, total):
    ...
    _notes(s,
        ...
        "honest one-line description, not marketing. The Digital Deputy "
...
def slide_problem(prs, total, page):
    ...
    b1 = _card(s, Inches(0.6), Inches(1.7), Inches(6.05), Inches(5.2),
               ...
               body_lines=[
                   ...
                   ("Manual GitLab issue creation", "with frequent duplicates"),
    ...
    b2 = _card(s, Inches(6.85), Inches(1.7), Inches(6.05), Inches(5.2),
               ...
               body_lines=[
                   ...
                   ("GitLab issues filed every 2 hours", "with two-layer dedup"),
    ...
    _notes(s,
        ...
        "until somebody remembered to file them, GitLab issues with "
...
def slide_two_dimensions(prs, total, page):
    ...
    _card(s, Inches(0.6), Inches(1.7), Inches(6.05), Inches(5.2),
          ...
          body_lines=[
              ("Requirement Management —", "03:15 BDT, IMAP + Drive ingest, GitLab issues with dedup"),
...
def slide_design_constraints(prs, total, page):
    ...
    items = [
        ...
        ("TOOLS ARE IDEMPOTENT",
         "Re-running any workflow is safe. UID checkpoints, Drive file-ID tracking, GitLab title dedup, fuzzy memory dedup — all baked in.",
         TEAL),
...
def slide_sdk(prs, total, page):
    ...
    code = [
        ...
        '    "Returns recent N rows with gitlab_issue_url if present.",',
...
def slide_tool_contract(prs, total, page):
    ...
    items = [
        ...
        ("HONOR NAPCO_NUCLEUS_DRY_RUN",
         "Mutating tools (publish_tasks_to_gitlab, send_email, git_commit_and_push) check the env var and short-circuit.",
         CORAL),
        ("RETURN THE SDK ENVELOPE",
         'Every tool returns {"content": [{"type": "text", "text": json.dumps(...)}]}. The _text() helper enforces it.',
         GREEN),
        ("LAZY HEAVY IMPORTS",
         "imaplib, googleapiclient, requests for GitLab, Playwright runner — all imported inside the tool fn. Cold-start stays sub-second.",
         GOLD),
    ]
    y = Inches(1.7)
    h = Inches(1.0)
...
def slide_prompt_files(prs, total, page):
    ...
    rows = [
        ...
        ["requirement_management.md",
         "03:15 BDT — IMAP + Drive ingest → 3-hour task split → GitLab issues with two-layer dedup"],
...
def slide_integrations(prs, total, page):
    ...
    rows = [
        ...
        ["GitLab", "requests (REST v4)",
         "list_open_issue_titles + create_issue. PAT with api scope."],
...
def slide_tech_stack(prs, total, page):
    ...
    items = [
        ...
        ("EXTERNAL APIs", "IMAP · Google Drive · Groq Whisper · GitLab · TFS · MSBuild · IIS", PURPLE),
...
def slide_workflows(prs, total, page):
    ...
    rows = [
        ...
        ["6", "Requirement Management", "03:15",
         "IMAP + Drive ingest. 3-hour task split. GitLab issues with dedup"],
    ...
    _notes(s,
        ...
        "Management runs once at 03:15 to ingest anything that arrived "
...
def slide_demerits(prs, total, page):
    ...
    items = [
        ...
        ("No multi-tenant story", "Allowlists, paths, GitLab project ID hard-coded for MVP Access.", TEAL),
...
def slide_closing(prs, total, page):
    ...
    _add_text(box.text_frame,
              ...
              "Every client requirement filed in GitLab within two hours of being spoken on a call. "
    ...
    _add_text(qa.text_frame, "arzaman@ael-bd.com",
    ...
    _notes(s,
        ...
        "human-in-loop is preserved, NN files draft GitLab issues, "
