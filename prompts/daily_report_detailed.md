...
**Scope: 6 sections** — Requirement Management, API Functional, API Integration, API Load, MVP Access E2E, MVPAccess CICD. Audience is the full engineering team plus management leadership review.

The 4 test workflows all fire at 02:00 BDT and finish by ~04:00 BDT. Requirement Management fires twice daily (07:00 + 19:00 UTC). MVPAccess CICD fires at 22:00 BDT. By the time this report runs at 09:00 BDT all of yesterday-into-today's runs are in memory and on disk.
...
### 3. Compose the detailed report — 6 sections, one PDF

Each test section has full detail with a pie chart. Sections 1 and 6 are text-only. Write each section yourself, no f-string templates. Every claim anchored to a source.
...
## 1. Requirement Management — Today's Ingestion
...
Personnel) and Type (Task / Bug / updatedRequirements). Cite each WP
by id and title; link the OpenProject project URL
`http://172.16.205.123:8080/projects/mvp-access/work_packages`.

Then a "Pipeline status" sub-paragraph (verbatim framing):
- **Email ingestion (IMAP):** operational — N emails polled today,
  M ingested, K skipped (allowlist + dedup).
- **Google Drive ingestion:** operational — file polling enabled, X
  documents processed today (or "none today" if empty).
- **MS Teams integration:** planned. Power Automate bridge scaffolded,
  pending workflow activation.

If `requirements_seen` was reset or any publish errored, surface it
explicitly. 4 to 8 lines plus the pipeline-status sub-paragraph.>
...
## 6. MVPAccess CICD
<Verbatim framing for now (until prod credentials land):
"Pipeline is set up and tested locally. Production deployment is
...
If the workflow has actually started running successfully (post
credentials), replace with: 3 to 5 lines on last night's pull, build
duration, deployed commit hash, IIS health check verdict.>
```

**Test sections (2, 3, 4, 5) MUST each have a pie chart** — pass / fail / skipped breakdown. Sections 1 (Requirement Management) and 6 (CICD) are text-only.
